...
- `specs/walk_curvature_speed.pmg_spec` is a genuine 2-D space (turn ×
...
- `specs/walk_jog.pmg_spec` is a genuine directed multi-node graph (walk ↔ jog
...
`walk_curvature_speed` carries the dimensionality story and `walk_jog` carries
the topology story.
...
- `walk_curvature_speed` declares `expect walk_2d spanned_axes 2` (true: both
...
- `walk_minimal` declares `expect walk corner_coverage full` because its 1-D
...
- If a tight-turn jog clip (and ideally a distinct third Group-B-compatible
  ...
  `walk_curvature_speed` to the fourth corner and add the
